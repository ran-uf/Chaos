...
class LSTMCells(torch.nn.Module):
    ...
    def forward(self, x, state):
        ...
        if state is None:
            h_0 = torch.zeros(batch_size, self.num_layers, self.hidden_size).to(x.device)
            c_0 = torch.zeros(batch_size, self.num_layers, self.hidden_size).to(x.device)
        else:
            (h_0, c_0) = state
